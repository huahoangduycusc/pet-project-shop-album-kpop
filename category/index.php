...
$product = new product();
?>
<div class="page_title"><a href="<?php echo $homeurl;?>">Trang chủ</a> > <a href="#"><?php echo $res['category_name'];?></a></div>
...
switch($do)
{
    case 'list':
        if($from !=0 || $to != 0)
        {
        $sql = "SELECT * FROM `product` WHERE `category_id` = '$id' AND `product_price` >= '$from' AND `product_price` <= '$to' ORDER BY `product_id` 
        DESC LIMIT $start, $limit";
        }
        if($from == 0)
        {
        $sql = "SELECT * FROM `product` WHERE `category_id` = '$id' AND `product_price` <= '$to' ORDER BY `product_id` 
        DESC LIMIT $start, $limit";
        }
        if($to == 0)
        {
        $sql = "SELECT * FROM `product` WHERE `category_id` = '$id' AND `product_price` >= '$from' ORDER BY `product_id` 
        DESC LIMIT $start, $limit";
        }
...
?>
<?php
$product->getList($start,$limit,$id);
?>
<!-- Section new product end -->
    <?php
    $duy = "SELECT * FROM `product` where `category_id` = '$id'";
    $demtrang = mysqli_num_rows(mysqli_query($con,$duy));
                $config = [
                    'total' => $demtrang,
                    'querys' => $id,
                    'limit' => $limit,
                    'url' => 'category/index.php?id'
                ];
