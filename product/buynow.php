...
<div class="wrap">
    ...
    $sqmua = "SELECT * FROM `product` WHERE `product_id` = '$id' LIMIT 1";
    if(!mysqli_num_rows(mysqli_query($con,$sqmua)))
    {
        chuyenhuong();
    }
    $sp = mysqli_fetch_assoc(mysqli_query($con,$sqmua));
    $soluong = isset($_GET['soluongsp']) ? abs(intval($_GET['soluongsp'])) : 1;
    if($soluong > $sp['quantity'])
    {
        chuyenhuong();
    }
    if(saleoff($sp['product_id']))
    {
        $tongtien = khuyenmai($sp['product_id'])*$soluong;
        $gia = khuyenmai($sp['product_id']);
    }
    else
    {
        $tongtien = $sp['product_price']*$soluong;
    ...
    ?>
        <div class="cart_left">
            ...
            if(isset($_POST['dathang']))
            {
                ...
                $total_price = $g['ship_price']+$tongtien-$giamgia; // cong tien ship va tien tat ca san pham, tru tien ma qua tang neu co
                ...
                values (null,'$fname','$phone','$timestamp','0','$dc','$total_price','$user_id','$method','0','$timeSql')";
                    if(mysqli_query($con,$sql))
                    {
                        ...
                        if(mysqli_num_rows($result))
                        {
                                ...
                                $sqll = "INSERT INTO `orderdetails` values (null,'".$res['product_id']."','$rid','$soluong','$gia')";
                                mysqli_query($con,$sqll);
                                $sql = "UPDATE `product` set `quantity` = `quantity` -'$soluong' where `product_id` = '".$res['product_id']."' limit 1";
        ...
        <div class="cart-right">
                ...
                if(mysqli_num_rows($result))
                {
                    $res = mysqli_fetch_assoc($result);
                         ...
                         <td class="hinh_sp">
                             ...
                             <span class="sp_soluong">'.$soluong.'</span>
                         </td>
                         <td class="sp_ten">&#160; '.$res['product_name'].'</td>';
                        echo'
                        <td> '.number_format($soluong*$res['product_price']).'VND';
                        if(saleoff($res['product_id']))
                        {
                            echo'<br><font color="red">'.number_format(khuyenmai($res['product_id'])*$soluong).'VND</font>';
                        }
                        echo'</td>
                     </tr>';
                }
            ...
            <div class="cart_line">
                Tạm tính <span class="text-right"><?php echo number_format($soluong*$sp['product_price']);?> VND</span>
            </div>
            <div class="cart_line">
            ...
            <div class="price_total">
                Thành tiền
                <span class="text-right"><span id="tt"><?php echo number_format($tongtien);?></span> VND</span>
            </div>
        </div>
    </div>
</body>
...
document.getElementById("ship").onchange = function(event) {
    ...
    var sotien = <?php echo $tongtien;?>;
    var tong = sotien + phi;
    if(Number.isNaN(tong))
    tong = <?php echo $tongtien;?>;
